...
def page_name_validator(page_dict, context):
    ...
    try:
        existing_page = logic.get_action('page_show')(
            context, {'id': page_dict.get('name')}
        )
        if (
            existing_page
            and existing_page.get('id') != page_dict.get('id')
            and page_dict.get('id') != page_dict.get('name')
        ):
            raise logic.ValidationError(
                {'name': [_('Page name already exists')]}
            )
