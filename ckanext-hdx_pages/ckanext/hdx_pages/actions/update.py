...
def process_groups(context, page, groups):
    ...
    for grp_id in grp_ids:
        assoc = pages_model.PageGroupAssociation.get(
            page_id=page.id, group_id=grp_id
        )
        assoc.delete()
...
def process_tags(context, page, tags):
    ...
    for tag_id in tag_ids:
        assoc = pages_model.PageTagAssociation.get(
        ...
        for tag in tags_vocabulary:
            ...
            if tag_id:
                pages_model.PageTagAssociation.create(
                    page=page, tag_id=tag.get("id"), defer_commit=True
                )
            else:
                raise logic.ValidationError(
