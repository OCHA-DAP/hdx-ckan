...
def page_create(context, data_dict):
    ...
    try:
        ...
        for grp_id in data_dict.get('groups', []):
            ...
            if pages_model.PageGroupAssociation.exists(
                ...
                raise toolkit.ValidationError(
                    ...
                    error_summary='The group, {0}, is already in the page'.format(
                        group_dict.get('id')
                    ),
                )
        ...
        for tag in tags_vocabulary:
            ...
            if tag_id:
                # We validate for id duplication, so this shouldn't be true during create.
                if pages_model.PageTagAssociation.exists(
                    page_id=page.id, tag_id=tag_id
                ):
                    raise toolkit.ValidationError(
                        'Tag already associated with page.',
                        error_summary='The tag, {0}, is already in the page'.format(
                            tag_id
                        ),
                    )
                # create the association
                pages_model.PageTagAssociation.create(
                    page=page, tag_id=tag_id, defer_commit=True
                )
            else:
                raise logic.ValidationError(
                    {'tag_string': ['Tag %s not found' % tag.get('name')]}
                )
        ...
        raise logic.ValidationError(
            {
                'message': [
                    'Something went wrong while processing the request: %s'
                    % msg
                ]
            }
        )
