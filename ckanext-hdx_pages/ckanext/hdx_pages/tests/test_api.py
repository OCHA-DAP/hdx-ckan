...
class TestHDXApiPage(object):
    def test_page_api(self):
        ...
        elnino = _get_action("page_show")(
            context_sysadmin,
            {"id": page_dict.get("id") or page_dict.get("name")},
        )
        assert elnino
        ...
        assert (
            "show_title" in elnino.get("extras")
            and elnino.get("extras").get("show_title") == "on"
        )
        assert (
            "https://data.humdata.org/dataset/wfp-and-fao-overview"
            in elnino.get("sections")
        )

        try:
            _get_action("page_update")(
                context_sysadmin, {"id": page_dict.get("id")}
            )
            assert False
        except Exception:
            assert True

        grp_dict = _get_action("group_show")(
            context_sysadmin, {"id": LOCATION}
        )
        new_page_dict = _get_action("page_update")(
            context_sysadmin,
            {
                "name": page_elpico.get("name"),
                "id": page_dict.get("id"),
        ...
        assert (
            "show_title" in new_page_dict.get("extras")
            and new_page_dict.get("extras").get("show_title") == "off"
        )
        ...
        try:
            _get_action("page_update")(
                context, {"title": "El Pico Lorem Ipsum"}
            )
            assert False
        except logic.NotAuthorized:
            assert True
        ...
        try:
            _get_action("page_delete")(
                context, {"id": page_dict.get("id") or page_dict.get("name")}
            )
            assert False
        except logic.NotAuthorized:
            assert True
...
class TestHDXPageWithGroups(object):
    def test_page_with_groups(self):
        ...
        page_group_list = _get_action("page_group_list")(
            context_sysadmin, {"id": page_dict.get("id")}
        )
        ...
        group_page_list = _get_action("group_page_list")(
            context_sysadmin, {"id": grp_dict.get("id")}
        )
...
class TestHDXPageShow(object):
    def test_page_show(self):
        ...
        eldeleted = _get_action("page_show")(
            context_sysadmin, {"id": page.get("id")}
        )
