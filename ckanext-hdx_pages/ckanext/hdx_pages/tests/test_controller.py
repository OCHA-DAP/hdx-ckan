...
class TestHDXControllerPage(object):
    def _get_url(self, app, url, apitoken=None):
        if apitoken:
            page = app.get(
                url,
                headers={
                    'Authorization': unicodedata.normalize(
                        'NFKD', apitoken
                    ).encode('ascii', 'ignore')
                },
    ...
    def _get_token_for_user(self, username):
        return factories.APIToken(user=username, expires_in=2, unit=60 * 60)[
            'token'
        ]


@pytest.mark.usefixtures(
    'keep_db_tables_on_clean',
    'clean_db',
    'clean_index',
    'setup_user_data',
...
class TestHDXControllerPageNew(TestHDXControllerPage):
    ...
    def test_page_new(self, app):
        ...
        try:
            ...
            assert (
                'Page not found' in page_new.body
            ), 'a regular user can not not access a page creation form'
...
class TestHDXControllerPageEdit(TestHDXControllerPage):
    ...
    def test_page_edit(self, app):
        ...
        url = h.url_for('hdx_custom_page.edit', id=page_dict.get('id'))
        try:
            page_edit = self._get_url(app, url)
            assert (
                'Page not found' in page_edit.body
            ), 'a regular user can not not access a page edit form'
...
class TestHDXControllerPageRead(TestHDXControllerPage):
    ...
    def test_page_read(self, app):
        ...
        assert 'Lorem Ipsum is simply dummy text' in eldashbo.get(
            'description'
        )

        context['user'] = SYSADMIN
        url = h.url_for('hdx_event.read_event', id=elnino.get('id'))

        elnino_result = self._get_url(app, url)
        assert '200' in elnino_result.status

        url = h.url_for('hdx_dashboard.read_dashboard', id=eldashbo.get('id'))
        eldashbo_result = self._get_url(app, url)
        assert '200' in eldashbo_result.status

        try:
            url = h.url_for('hdx_event.read_event', id='nopageid')
            eldashbo_result = self._get_url(app, url)
            assert (
                'Page not found' in eldashbo_result.body
            ), "page doesn't exist"
    ...
    def test_page_read_hidden_title(self, app):
        ...
        elescondite = _get_action('page_create')(
            context_sysadmin, page_elescondite
        )
        assert elescondite
        assert 'El Escondite' in elescondite.get('description')
        assert (
            'show_title' in elescondite.get('extras')
            and elescondite.get('extras').get('show_title') == 'off'
        )

        url = h.url_for('hdx_event.read_event', id=elescondite.get('id'))
        elescondite_page = self._get_url(app, url)
        assert (
            '<h1 class="itemTitle">El Escondite</h1>'
            not in elescondite_page.body
        )
...
class TestHDXControllerPageDelete(TestHDXControllerPage):
    ...
    def test_page_delete(self, app):
        ...
        try:
            eldeleted_dict = _get_action('page_create')(
                context_sysadmin, page_eldeleted
            )
            assert eldeleted_dict
            assert 'El Deleted' in eldeleted_dict.get('title')
            assert 'eldeleted' in eldeleted_dict.get('name')
            assert 'El Deleted Lorem Ipsum' in eldeleted_dict.get(
                'description'
            )
        except Exception:
            assert False

        eldeleted_page = _get_action('page_show')(
            context_sysadmin, {'id': page_eldeleted.get('name')}
        )
        try:
            url = h.url_for(
                'hdx_custom_page.delete_page', id=eldeleted_page.get('id')
            )
        ...
        try:
            url = h.url_for('hdx_custom_page.delete_page', id='nopageid')
