...
class TestRequestDataEmails(hdx_test_with_inds_and_orgs.HDXWithIndsAndOrgsTest):
    ...
    @mock.patch('ckanext.hdx_package.actions.get.hdx_mailer.mail_recipient')
    def test_archive_request(self, mocked_mail_recipient):
        member = model.User.by_name('tester')
        member.email = 'tester@test.com'
        ...
        package = {
            ...
            "name": "requestdata_package_2_for_emails",
            "notes": "This is a test activity",
            "title": "Requestdata package 2 for emails",
            "groups": [{"name": "roger"}],
            "owner_org": self.ORG_NAME,
            "maintainer": self.ADMIN_USER,
            "is_requestdata_type": True,
            "file_types": ["csv"],
            "field_names": ["field1", "field2"]
        }
        pkg_dict = get_action('package_create')(context, package)
        ...
        assert 'is archived' in args[1]
        assert 'email/content/request_data_auto_approval_to_admins.html' == kw_args.get('snippet')
