...
class TestResponsiveRedirect(hdx_test_with_inds_and_orgs.HDXWithIndsAndOrgsTest):
    ...
    def test_responsive_redirect(self):
        ...
        result = test_client.get(desktop_url, headers={'User-Agent': MOBILE_UA})
        assert result.status_code == 200, 'Desktop page should always render, regardless of user agent'

        result = test_client.get(light_url)
        assert result.status_code == 301, '/m/ pages should always permanently redirect to the desktop page'

        result = test_client.get(light_url + '?force_layout=desktop')
        assert result.status_code == 301, '/m/ pages should redirect regardless of the force_layout param'
