...
- `fanstatic/v2/components/dataset-card.js` — no changes needed
...
- `v2/components/dataset-card.js` — description expand/collapse; already works, no changes
...
| Description expand/collapse | `v2/components/dataset-card.js` via `data-module="dataset-card"` | Component already wired; no change needed |
...
2. Spot-check data mapping:
   ...
   - Multi-country dataset → "Multiple locations" label
   - No-country dataset → no location row
   - `package.private = True` → lock badge in formats
   - `cod_level = 'cod-enhanced'` → "COD+" dark badge in formats
   - `batch_length > 0` → footer "View other N datasets" visible

3. Functionality:
   - "Show more" click → description expands/collapses
   - Title click → navigates to dataset page
   - Org name click → navigates to org page
   - Trigger a search → Mixpanel `search` event fires

4. Responsiveness:
   - SM (< 48rem): description hidden, formats hidden, subnational hidden; location + date side-by-side
   - MD (48–80rem): full two-column layout
   - XL (≥ 80rem): full two-column layout with fixed left-column width

5. Edge cases:
   - No-description dataset → no "Show more" button
   - No-format dataset → no formats row
   - 5+ format dataset → `+N` overflow badge shown
...
| Purpose | Path |
|---|---|
| Current package item | `ckanext-hdx_theme/ckanext/hdx_theme/templates/search/snippets/package_item.html` |
| Current package list | `ckanext-hdx_theme/ckanext/hdx_theme/templates/search/snippets/package_list.html` |
| Current package icons | `ckanext-hdx_theme/ckanext/hdx_theme/templates/search/snippets/package_icons.html` |
| v2 dataset-card template | `ckanext-hdx_theme/ckanext/hdx_theme/templates/v2/components/dataset-card.html` |
| v2 dataset-card LESS | `ckanext-hdx_theme/hdx-styles/src/common/less/v2/components/dataset-card.less` |
| v2 dataset-card JS | `ckanext-hdx_theme/ckanext/hdx_theme/fanstatic/v2/components/dataset-card.js` |
| Homepage v2 (migration pattern) | `ckanext-hdx_theme/ckanext/hdx_theme/templates/home/index.html` |
| v2 page base | `ckanext-hdx_theme/ckanext/hdx_theme/templates/v2/page.html` |
| Figma export (list) | `ckanext-hdx_theme/llm_docs/redesign/figma_exports/dataset-list.html` |
| Figma export (card) | `ckanext-hdx_theme/llm_docs/redesign/figma_exports/dataset-card.html` |
| Design tokens | `ckanext-hdx_theme/ckanext/hdx_theme/fanstatic/v2/foundation.css` |
| Layout CSS | `ckanext-hdx_theme/ckanext/hdx_theme/fanstatic/v2/layout.css` |
| Breakpoints LESS | `ckanext-hdx_theme/hdx-styles/src/common/less/v2/breakpoints.less` |
...
| Comparable task | `ckanext-hdx_theme/llm_docs/redesign/requirements/029-implement-dataset-card-component.md` |
