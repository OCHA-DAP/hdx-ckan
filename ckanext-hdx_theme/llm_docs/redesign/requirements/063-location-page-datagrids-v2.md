...
| D19 | The 6 category cards are **not rendered visibly by default at all** — the whole `__grid` sits inside one native `<details data-datagrid-reveal>`, closed by default. A single "Show more"/"Show less" control (its `<summary>`, styled as a `c-text-button`) sits on its own row, left-aligned, directly below `__top`. The KPI row + stacked-bar chart + legend (§2.3–§2.4) stay visible unconditionally. Clicking it expands every category card's sub-categories at once, driven by the reveal's native `toggle` event, and collapses everything again on a second click. | Requester's explicit choice. A native `<details>`/`<summary>` for the outer reveal (not a CSS-hidden block controlled by a plain button) keeps the grid reachable with JavaScript disabled, matching the per-card `<details>` pattern (§9). |
| D20 | "Definitions"/"Overview of Data Grids" (`&__buttons-group`) sit inside `[data-datagrid-reveal]`, after `&__grid` — collapsed by default along with the category cards, appearing below them once expanded. | Requester's explicit choice. |
...
has zero N/A sub-categories, the page-level chart's N/A legend item is hidden rather than always shown (D6).
...
|---|---|---|
| Expand/collapse (whole grid) | `<input type=checkbox>` + jQuery `.toggle()` on all `.data-item-details` | Single native `<details data-datagrid-reveal>`/`<summary>` (D19) wrapping `__grid` and `__buttons-group` ("Definitions"/"Overview of Data Grids", D20) together, closed by default — nothing in either renders visibly until "Show more" is clicked. The `<summary>` is styled as a `c-text-button`, left-aligned on its own row directly below `__top`. `location-page.js` listens for the reveal's native `toggle` event and forces every category card's own `<details>` `open` property to match, so opening/closing the reveal still expands/collapses every card's sub-categories at once (checkbox and jQuery replaced, same single-toggle-for-everything behavior preserved, per §2.5's confirmed Figma reading) |
...
| Zone | SM | MD | XL |
|---|---|---|---|
| Category cards | Vertical stack, 1 col; whole grid hidden until "Show more" is clicked (D19) | Vertical stack, 1 col; whole grid hidden until "Show more" is clicked (D19) | 3-col × 2-row grid (flex/grid-fr, not fixed px — see §2.6); whole grid hidden until "Show more" is clicked (D19) |
...
| JS disabled | The whole grid is hidden by default (D19) via a native `<details data-datagrid-reveal>`, so clicking its `<summary>` ("Show more") still reveals it with zero JavaScript. Each category card is, in turn, its own native `<details>`/`<summary>`, collapsed by default (a deliberate departure from v1's checked-by-default checkbox) — it still opens on click with zero JavaScript too, so every category stays individually reachable. What *does* require JS: forcing every card open at once when the reveal is opened (D19) — without JS, a visitor must open each card individually after revealing the grid. **Round 5 caveat:** the desktop click-disable is itself JS (`preventDefault` in `location.js`), so a JS-disabled desktop visitor regains per-card click — an accepted trade-off, not a bug |
...
|---|---|---|
| Template | `ckanext-hdx_theme/ckanext/hdx_theme/templates/country/country.html` | Re-parented to `v2/page.html`; header + Data Grid section + commented-out Key Figures/map block (D5) all in `{% block pre_primary %}`. `__grid` and `__buttons-group` ("Definitions"/"Overview of Data Grids") both sit inside a single `<details data-datagrid-reveal>`/`<summary>` directly below `__top` (D19, D20) — hand-rolled `c-text-button` markup on the `<summary>` itself, not the shared `text-button.html` snippet, since nesting a real `<button>`/`<a>` inside `<summary>` isn't valid |
| Template | `ckanext-hdx_theme/ckanext/hdx_theme/templates/light/group/read.html` | Unchanged, out of scope for now |
| Template | `ckanext-hdx_theme/ckanext/hdx_theme/templates/v2/completeness-item.html` | Single category-card partial, called directly by `country.html`; a native `<details>`/`<summary>`, collapsed by default, so clicking a card expands/collapses it independent of the page's reveal toggle. The desktop click-disable is applied in JS/CSS only (see `location.js`/`location.less` rows below); the outer grid-visibility reveal (D19) wraps this partial from `country.html` without touching its own markup |
...
| LESS | `hdx-styles/src/common/less/v2/pages/location.less` | Page-specific (per `-page.less` naming convention). Card summary's `cursor: pointer` is scoped to `<@hdx-bp-md`, `cursor: default` at `≥@hdx-bp-md`; the card's `:hover` border-highlight is likewise scoped to `<@hdx-bp-md` only. `&__reveal-summary` carries the same `<details>` marker reset used on the per-card `&__details`; the icon-rotate rule is keyed off `&__reveal[open]`. At XL, `&__chart-panel` is content-sized (`flex: 0 0 auto`, so its box always matches its own swatch-bar/legend content) and right-aligned via `&__stats-chart`'s `justify-content: space-between`; `&__kpi-row` (`flex: 1`) absorbs the rest of the row |
...
| JS | `ckanext-hdx_theme/ckanext/hdx_theme/fanstatic/v2/pages/location.js` | Expand/collapse is driven by a single `[data-datagrid-reveal]` `<details>` element's native `toggle` event (fires on both a real click and a scripted `.open` change), forcing every card's native `<details>` open/closed to match and swapping the "Show more"/"Show less" label (D19). Drawer jump-nav scroll wiring (reuses `window.hdxSmoothScrollTo` from `anchor-links.js` against the drawer's own scroll container, offset by the `__sticky-top` block's rendered height); a self-contained `MutationObserver` measuring the drawer's title-bar height into a `--drawer-header-height` custom property (D18). `initDesktopCardClickDisable()` — `matchMedia('(min-width: 48rem)')`-gated `preventDefault()` on each card summary's click, plus `tabindex` toggling, re-evaluated live via the `mql`'s `change` listener. Registered **capture-phase** (not bubble) so it fires before any inner element's own click handler — e.g. `tooltip.js`'s handler on the card title (`.c-tooltip-trigger`) calls `stopPropagation()`, which would otherwise block a bubble-phase listener on the summary from ever running when the click lands on the title |
