...
(matching 056 `read()` / 057 `activity_offset()` precedent — confirmed, Q1).
...
(`v2/org_hero.html` → `page-header.html` + `tabs.html`) **already renders a Members tab item**
...
— Q2/Q3/Q6), and all three pieces of live functionality Figma's sidebar omits are **kept**
(Pending approval, Leave this organisation, Group message — Q5/Q6/Q7).
...
else `members.html` — the same branch 056/057 removed for `read()`/`activity_offset()` (→ Q1).
...
added to org" does not exist anywhere in the data model (→ Q3).
...
- Avatar: `h.user_image(member.name, 20)` — **gravatar image**, not initials (→ Q11).
...
- Role line: `{{ translated_role }} - Registered {{ h.render_datetime(member.created) }}`,
  plus `- Sysadmin` suffix when `sysadmin` (→ Q16). `member.created` = **user registration
...
2. **Current-user card**: `h.user_image(name, 70)`, display name `|` username links,
   ...
   *change-role modal* (`#edit-member-div-…`), not the profile editor (→ Q8) — `|`
...
bar (Q15: not in scope — plain GET search, v1 parity) and a chevron-up collapse icon in the
...
breakpoint shows `|` — confirmed a Figma export artifact; `|` is canonical everywhere (Q13).
...
sidebar cards** — decided (Q14): the search bar and the stacked sidebar cards ARE included at
SM anyway, MD-style (feature parity; pagination is dropped at all breakpoints per Q2).
...
|---|---|---|
| Avatar | `__avatar` | reuse `v2/components/avatar.html` (`c-avatar`), initials-only (Q11 — gravatar dropped on this page) |
...
| Role + registered line | `__meta` | `translated_role` + `h.render_datetime(member.created)` (user registration date — the only date available, §1.2); bold `- Sysadmin` suffix rendered by the component from the separate `sysadmin` bool param (Q16) |
...
{% call h.snippet('v2/components/member-list-card.html',
    avatar_initials=...,                            {# initials-only, Q11 #}
    name=..., username=..., profile_url=...,
    meta_text=...,                                  {# pre-rendered "Admin - Registered …" line #}
    sysadmin=...,                                   {# bool — appends bold "- Sysadmin" (Q16) #}
...
- **States: default + `:hover`** (pseudo-class only — never an `is-hovered`/parameter). Figma
  ...
  `dataset-card`/`org-list-card` pattern) (Q17: confirmed).
- No JS of its own. WCAG: links are real `<a>`; action controls are `<button>`s (dropdown
  triggers for change-role/approve, drawer opener for remove — Q9); ellipsized
...
- **Pending-approval rows** (§1.6.3) share the avatar/name/date left side and reuse the card
  ...
  text button) with the `--stacked` variant for the narrow sidebar column (Q5: the Pending
...
- **The sidebar "Your role" bio card** is visually a borderless variant with a bio text block —
  NOT forced into this component; §6 keeps it as page-level markup reusing `c-avatar` (Q18:
  confirmed).
...
- **Search — reuse `v2/components/search-input.html`** (`c-search-input`, size `m`) inside a GET
  ...
  *"Search for members"* (Figma). Preserve `sort` as a hidden input so searching doesn't reset
  it (no pagination params — Q2). The magnifier is a submit button (`icon_submit=True`) and a
...
- **Sorting — server-side reload, reusing the v2 navigate-dropdown pattern.** The existing
  ...
  `ext_page_size` semantics, so it is **not directly reusable**; per Q4 it is **generalized** —
  parameterized to accept caller-provided option lists / param names, with the per-page dropdown
  optional (this page doesn't render one, Q2). Existing callers (search page, org datasets tab)
  keep their current behavior via defaults and **must be regression-checked**. No new dropdown
  component is built.
- **Sort options (decided, Q3):** Figma's default "Last added" is **not implementable** — the
...
**Decided (Q2): no pagination — v1 parity, a deliberate deviation from Figma.** The entire
role-grouped member list (Q6) renders on one page, exactly as today:
...
- No new GET params, no slicing in `members()`, in the template, or anywhere else; the view is
  untouched beyond the Q1 template-selection edit.
...
- **Card 1 — "Your role":** `Your role for this organisation: {role}` + current-user block
  ...
  nothing in the data model drives it (Q12). V1's **"Leave this organization"** action is
  **kept** in the card (Q7), and **"Edit profile" keeps its v1 change-role behavior** — the
  change-role UI is the one-click `c-dropdown--link` role dropdown (no dialog), rendered for
  every sidebar viewer (Q8/Q9).
- **Card 2 — "Add / invite colleagues":** gated by `allow_approve` (v1 parity). v2 build from
  ...
  the select2 tags-autocomplete is **ported to v2** (Q10) — new JS building on the existing
...
- **Pending approval:** absent from Figma but live functionality — **kept in its v1 position**
  (between the "Your role" card and the invite card), restyled with v2 primitives (Q5); the
...
- **Group message (list column, role-section headers) — kept (Q6):** the per-role "Group
...
|---|---|---|
| route-level `g.user` check | whole page (403 for anonymous) | unchanged; tab item already gated `show: c.user` in `org_hero.html` |
...
| `member_delete` auth override (self-removal) | "Leave this organization" | unchanged backend; UI kept in the bio card (Q7) |
...
- **Member / Editor:** list without row actions + sidebar Card 1 only (own card, leave-org
  kept per Q7). No invite card, no pending approvals.
...
template/asset-layer plus the Q1 template-selection edit in `members()` (no slicing — Q2
dropped pagination).
...
| Org hero (header + tabs) | **Reuse as-is** — `v2/org_hero.html` with `active_tab='members'`, same `header_stats` | Zero new params needed; Members item already in the tabs list |
| Tabs | **Reuse as-is** — `v2/components/tabs.html` (via org_hero) | — |
| Search bar | **Reuse** — `c-search-input` in a GET form (§4) | Existing component matches Figma's input 1:1 |
| Sort by (no per-page — Q2) | **Generalize** `v2/search-nav-controls.html` (Q4): caller-provided option lists / param names; per-page dropdown optional | Existing callers (search page, org datasets tab) keep behavior via defaults — regression-check both |
...
| Avatar | **Reuse** — `c-avatar`, initials-only (Q11), no badge (Q12) | — |
| Pagination | **Not implemented** (Q2) — full list, v1 parity | §5 |
...
| Remove/leave confirmation + group-message dialogs | **Reuse `c-drawer`** (`v2/components/drawer.html` + `fanstatic/v2/components/drawer.js`) with explicit Confirm/Cancel | Change-role is NOT a dialog — one-click `c-dropdown--link` (Q9); drawer is the established pattern (contact contributor, 051) |
| Invite input | **Port** — tags-autocomplete rebuilt on `c-autocomplete` (045) + new v2 JS; source `/util/user/hdx_autocomplete`, comma tokenizer (Q10) | Keeps the username/email lookup convenience without select2 |
| Group message popup | **Rebuild in `c-drawer`** + v2 form primitives + reCAPTCHA (Q6) | Replaces the v1 select2/reCAPTCHA popup; links stay on role-section headers |
| Sidebar bio card | Page-level markup (`hdx-v2-org-members-*`), composing `c-avatar` + text links (Q18: confirmed) | One-off; promote to a component only if reused later |
...
`url-nav.js`, which the generalized nav controls keep using (Q4 — matching how the datasets tab
...
|---|---|---|---|---|
| **XL (≥ 80rem)** | Two columns (list 53.5rem-equivalent + right sidebar), gap 1.5rem | Title+count left, "Sort by" right (no per-page — Q2), in one row above the search bar | Horizontal: info col + right-aligned actions | Right column: Your role card, pending approvals, invite card |
...
| **SM (< 48rem)** | Single column | Same order as MD (Q14: search bar included) | **Stacked**: info block, then actions row left-aligned below | Stacked below the list (Q14: included) |

- Member cards render inside the v1 role-grouped sections (Q6) at all breakpoints.
...
| **Losing live functionality Figma omits** ❗ — Pending approval, Leave organisation, Group message, sysadmin suffix, select2 invite autocomplete | All explicitly decided (Q5–Q10, Q16): **everything is kept** — Pending approval (v1 position), Leave, Group message (drawer rebuild), sysadmin suffix, invite autocomplete (v2 port) |
| **Inconsistent member rendering** ❗ — role-grouped (v1) vs flat (Figma) lists diverge in counts, headers, group-message anchors | Resolved (Q6): role-grouped v1 structure kept; count semantics (`members|length` = filtered count) preserved |
| **Pagination mismatch** ❗ — full-list rendering today vs numbered pager in Figma | Resolved (Q2): no pagination — v1-parity full list, no slicing anywhere; deliberate Figma deviation |
| **Generalizing `search-nav-controls.html`** (Q4) touches the search page + org datasets tab | Backwards-compatible defaults for existing callers; regression-check both pages after the change |
| **Invite autocomplete port** (Q10) — new v2 JS replacing select2 on a POST-critical form | Build on `c-autocomplete` (045); behavior-parity checklist: comma tokenizing, email + username entries, unchanged `emails` field payload to `bulk_member_new` |
| **Duplication of components** ❗ — user-row/bio-card/approval-row look-alikes | §3 reusability check done up front; single `member-list-card` with caller-provided actions; bio card stays page-level (Q18) |
...
| Performance on large orgs — `hdx_get_org_member_info` per rendered row (N+1) | No pagination (Q2) — v1-parity full-list enrichment; flagged, not worsened |
| Drawer-based dialogs (Q9) carry destructive actions (remove member) | Explicit Confirm/Cancel step mandatory in the drawer; keyboard/focus management per WCAG checklist |
...
|---|---|
| Search with zero matches | List area renders an empty-state message (Q17: search-page empty-state pattern, "No members found" copy); count shows `0`; sidebar unaffected |
| Org where the viewer has no role and isn't sysadmin | Full-width list, no sidebar, no actions (v1 parity) |
| Sysadmin viewing an org they're not a member of | Sidebar visible; `current_user.role` is `None` — v2 renders the raw role value like v1 (empty for sysadmin non-members — Q17) |
| Last non-sysadmin admin | Row actions replaced by the "add another admin" note; "Leave" (kept, Q7) blocked with the existing message |
| Member who maintains datasets in the org | Cannot be removed; `member` role option disabled (with tooltip) in the change-role dropdown; the dataset list is shown in the remove drawer (v1 behavior, Q9) |
...
| Missing avatar | Always initials (Q11: initials-only — no image path on this page) |
| Large orgs (hundreds of members) | Full list rendered (Q2: no pagination) — v1 parity |
| Zero pending requests / non-approver | Pending block absent entirely (not an empty shell) |
| Sysadmin member rows | `- Sysadmin` suffix kept (Q16) |
| `is_custom = True` org | Same unified template as standard orgs (Q1: confirmed, 056/057 precedent) |

---

## 12. Decisions (2026-07-12)

All open questions were resolved with the product owner; the decisions below are final for this
task and are folded into §§2–11 (Q-numbers are kept as reference ids used throughout the doc):

| # | Decision |
|---|---|
| **Q1** | Unify — remove the `is_custom` branch in `members()` (`views/members.py:112-115`); `custom_members.html` left orphaned like `custom_activity_stream.html` (057 D9) |
| **Q2** | **No pagination** — v1 parity, deliberate Figma deviation; no "Results per page" dropdown, no view/template slicing, no new GET params |
| **Q3** | Sort ships **Name Ascending / Descending only** (existing `title asc`/`title desc`), default ascending; "Last added" dropped (no timestamp in the data model) |
| **Q4** | **Generalize `v2/search-nav-controls.html`** — caller-provided option lists / param names, per-page dropdown optional; regression-check the search page + org datasets tab |
| **Q5** | **Pending approval kept, v1 position** (sidebar, between "Your role" and the invite card), restyled with v2 primitives; AJAX + analytics contract (§1.8) preserved |
| **Q6** | **Role-grouped v1 list structure kept** ("ADMINS [N]"-style headers); **Group message kept** — popup rebuilt as a `c-drawer` (contact-contributor pattern) in this task |
| **Q7** | **"Leave this organization" kept** in the bio card; last-admin block message follows it |
| **Q8** | "Edit profile" **keeps the v1 change-role behavior** as the one-click change-role dropdown — no modal/dialog; rendered for every sidebar viewer (v1 parity) |
| **Q9** | **`c-drawer` for confirmations** — remove/leave and the group-message form, with explicit Confirm/Cancel; **change-role and approve ship as one-click `c-dropdown--link` dropdowns** (selection applies immediately: hidden-form POST to `member_new` / AJAX `member_request_process`) |
| **Q10** | **Invite tags-autocomplete ported to v2** — new JS on top of `c-autocomplete` (045); same `/util/user/hdx_autocomplete` source, comma tokenizer, email support |
| **Q11** | **Initials-only avatars** (`c-avatar`); gravatar dropped on this page |
| **Q12** | Figma bio-card avatar **badge omitted** — nothing in the data model drives it |
| **Q13** | MD `+1` is an **export artifact** — `\|` separator is canonical at all breakpoints |
| **Q14** | Search bar + stacked sidebar cards **included at SM** anyway, MD-style (feature parity) |
| **Q15** | Member-search autocomplete **out of scope** — plain GET search, v1 parity; the hidden Figma layer is a leftover from the global search component |
| **Q16** | **`- Sysadmin` suffix kept** (v1 parity) |
| **Q17** | Empty search: search-page empty-state pattern, "No members found" copy; hover: standard 028 card treatment; sysadmin non-member: "Your role…" renders the raw role value, empty when none — v1 parity |
| **Q18** | Sidebar bio card stays **page-level markup**; promote to a component only if another page needs it |
...
| `ckanext-hdx_theme/.../templates/organization/custom_members.html` | Orphaned per Q1 (kept in place, 057 D9 precedent) |
| `ckanext-hdx_theme/.../templates/v2/search-nav-controls.html` | Generalized per Q4 (caller-provided option lists / param names, per-page optional); search page + org datasets tab regression-checked |
...
| `ckanext-hdx_theme/.../fanstatic/v2/org-members-page.js` | Approve/decline AJAX port, change-role hidden-form POST wiring, remove/leave + group-message drawer wiring incl. invisible reCAPTCHA (Q6/Q9), invite tags-autocomplete JS (Q10) |
...
| `ckanext-hdx_org_group/.../views/members.py` | Q1 template unification only — no page slicing (Q2: no pagination). **No other view/auth/action changes** |
