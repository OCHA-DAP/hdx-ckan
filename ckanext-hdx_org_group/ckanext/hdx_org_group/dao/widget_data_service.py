...
def build_widget_data_access(country_dict):
    """
    :param country_dict: dictionary with country info
    :type country_dict: dict
    :return: instance of WidgetDataService
    :rtype: WidgetDataService
    """
    return WidgetDataService(
        country_name=country_dict.get('name'),
        activity_level=country_dict.get('activity_level')
    )


class WidgetDataService(object):

    def __init__(self, country_name, activity_level=None):
    ...
    def get_dataset_results(self):
        ...
        """
        Return top line data for a country.
        If activity_level == 'active' or None (not set), returns an empty list. If 'inactive', fetches data from indicators.
        """
        if self.activity_level == 'active' or self.activity_level is None:
            return []
