...
- **Every brand-new resource** — whether it has a genuine `upload` payload or is URL-only (no `upload` key at all) — is flagged with its real id after `model.Session.flush()`, once that id is known. "New" here is deliberately determined from this package's **currently active** resources only (excluding `state='deleted'` rows), NOT the same all-states set used for the validation-facing checks above — a previously-deleted resource id resurrected in the *same* package (e.g. re-added via a direct `package_revise(update__resources__extend=[...])` call) with no url/last_modified change at all is still flagged here as "new for datastore purposes", since its datastore table was already dropped the moment it was deleted (see `package_resource_list_save()`, `ckan/lib/dictization/model_save.py:110-121`) and needs resubmitting, even though CKAN core's own validation semantics correctly treat it as an existing (not new) row (see `test_package_revise_resurrected_deleted_resource_treated_as_datastore_new`). Format/allowlist eligibility is still fully decided inside `_manage_datastore_for_uploads()` itself, so it's safe to flag every new resource unconditionally here.
- A caller-supplied resource id that reuses a **deleted resource from a different package** gets two different treatments, matching the two separate id sets above:
  - For the pre-validation, existing-resource checks (real upload flagging, `url`/`last_modified` change flagging), it's treated as **existing** — via a targeted global id lookup extending `existing_resource_ids` — matching CKAN core's unfiltered `resource_dict_save()` lookup (see `test_package_update_cross_package_deleted_resource_id_flagged_pre_validation`).
  - For the post-flush "new for datastore" check (`resource_was_new`), it's still absent from *this* package's `active_resource_ids` (it belongs to — or was deleted from — a different package), so it's flagged as **new** there regardless of any other change, ensuring it's (re-)submitted for datastore evaluation (see `test_package_update_cross_package_deleted_resource_id_flagged_as_new`).
...
| `test_package_revise_resurrected_deleted_resource_treated_as_datastore_new` | A resource is soft-deleted, then the same id is re-added to the **same** package with an identical url/last_modified (no other change) via `package_revise(update__resources__extend=[...])` | `_manage_datastore_for_uploads` **called once**, with the resurrected resource's id in `context[FILE_WAS_UPLOADED]` (its datastore table was already dropped on delete and must be resubmitted) |
| `test_package_update_cross_package_deleted_resource_id_flagged_as_new` | A resource is soft-deleted on package A, then the same id is reused on **package B** with an identical url/last_modified (no other change) | The reused resource's id is added to `context[FILE_WAS_UPLOADED]` (absent from package B's `active_resource_ids`, so flagged as new for datastore purposes) |
