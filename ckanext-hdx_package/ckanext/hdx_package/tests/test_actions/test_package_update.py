    ...
    def test_hdx_package_update_metadata(self):
        ...
        for key, value in package.iteritems():
            if key not in modified_fields.keys():
                if key != 'groups':
                    assert modified_package[key] == value, 'Problem with key {}: has value {} instead of {}'.format(
                        key, modified_package[key], value)
            else:
                assert modified_package[key] == modified_fields[key], 'Problem with key {}: has value {} instead of {}'.format(
                    key, modified_package[key], modified_fields[key])
